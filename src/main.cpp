...
void save_name()
{
	std::ofstream save("name_save");
	save << golf.getSwings();
	//shots = golf.getSwings();
	save << "\n";
	save << input;
	//user_name = input;
	save.close();
...
void getUserName()
{
	...
	window.render(0, 0, startTexture);
	window.renderword("NEW RECORD!!! ", font, color, 350, 150);
	...
	window.renderword(input.c_str(), font2, color2, 320, 380);
	window.display();
...
int main(int argc, char* args[])
{
	...
	Mix_FreeChunk(golfhit);
	Mix_FreeChunk(goalhit);
